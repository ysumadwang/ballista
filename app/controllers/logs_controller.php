  ...
  function index($id = null, $dispatch = null) {
    ...
    if (empty($id)) {
      ...
      $this->paginate = array(
        ...
        'fields' => array(
          'id', 'user_id', 'instance_id', 'status', 'commit', 'branch', 
          'comment', 'updated', 'logtime', 'output', 'Instance.id', 
          'Instance.project_id', 'Instance.server_id', 'Instance.active', 
      ...
      $this->paginate = array(
        'conditions' => array('Instance.project_id' => $id), 
        'order' => array('logtime DESC', 'Log.id DESC')
      );
      $projects = $this->Log->Instance->Project->find(
        'list', array(
  ...
  function execute($id = null) {
    if (!empty($this->data)) {
      ...
      foreach ($this->data['Log'] as $key => $val) {
        ...
        $check = $this->Log->query("
          SELECT * FROM instances_groups 
          WHERE instance_id = '" . $val['instance_id'] . "' 
          AND group_id IN (" . implode(',', array_keys($groups)) . ")"
        );
        if (empty($check)) {
